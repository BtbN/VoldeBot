...
class IRCBot(object):
    ...
    def _ping_decorator(self, func):
        def inner(nick, message, channel, **kwargs):
            message = re.sub('^%s[:,\s]\s*' % self.conn.nick, '', message, flags=re.I)
...
def run_bot(bot_class, host, port, nick, channels=None, password=None):
    """\
    Convenience function to start a bot on the given network, optionally joining
    some channels
    """
    conn = IRCConnection(host, port, nick, password)
